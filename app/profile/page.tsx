...
//   console.log('ProfilePage rendered. Current user state:', user)
//   // Create & Join States
//   const [newGroupName, setNewGroupName] = useState('')
//   const [isCreating, setIsCreating] = useState(false)

//   const [isJoinMode, setIsJoinMode] = useState(false) // জয়েন ফর্ম দেখানোর জন্য
...
//           if (tasksError) {
//             console.error('❌ টাস্ক লোড হতে সমস্যা:', tasksError.message)
//             // যদি relation error আসে, তাহলে বিকল্পভাবে শুধু ডাটা আনবো
...
//             if (fallbackTasks) fetchedTasks = fallbackTasks
//           } else if (tasksData) {
//             fetchedTasks = tasksData
//           }
//         }

//         setTasks(fetchedTasks as TaskRecord[])
//         setUser({
//           ...userData,
//           groups: groupInfo,
//         } as UserProfile)
...
//   // নতুন State: কোন টাস্কটা কমপ্লিট হচ্ছে সেটার লোডিং দেখানোর জন্য
//   const [completingTaskId, setCompletingTaskId] = useState<string | null>(null)

//   // 🌟 Task Complete Logic (With Proxy System)
//   const handleCompleteTask = async (taskId: string, assignedToId: string) => {
//     if (!user) return
//     setCompletingTaskId(taskId)

//     try {
//       // ১. টাস্কটিকে 'completed' করা এবং কে কমপ্লিট করেছে তার ID বসানো
...
//       // ২. প্রক্সি লজিক: যদি অন্য কেউ কাজ করে দেয়, তবে তাকে Skip Credit দেওয়া
//       if (user.id !== assignedToId) {
//         const newCredit = (user.skip_credit || 0) + 1

//         const { error: userError } = await supabase
//           .from('users')
//           .update({ skip_credit: newCredit })
//           .eq('id', user.id)
...
//   // 🌟 Skip Task Logic (with Auto-Assign to next member)
//   // 🌟 Skip Task Logic (Fixed)
//   const handleSkipTask = async (taskId: string, groupId: string) => {
//     if (!user || user.skip_credit <= 0) return
//     setCompletingTaskId(taskId)

//     try {
//       // ১. ইউজারের ক্রেডিট ১ কমিয়ে দেওয়া
//       await supabase
//         .from('users')
//         .update({ skip_credit: user.skip_credit - 1 })
//         .eq('id', user.id)

//       // ২. গ্রুপের ডেটা নিশ্চিতভাবে আনা
...
//       // ৩. পরের সিরিয়ালের জনকে খুঁজে বের করা
//       const nextSerial =
//         user.group_serial >= groupData.total_members ? 1 : user.group_serial + 1
...
//         .maybeSingle() // maybeSingle দিলে যদি ইউজার নাও থাকে, এরর দিবে না

//       // ৪. টাস্কটির দায়িত্ব পরের জনকে দিয়ে দেওয়া (যদি পরের জন থাকে)
//       if (nextUser) {
//         await supabase
//           .from('tasks')
//           .update({
...
//           .eq('id', taskId)
//       } else {
//         // যদি মেম্বার না থাকে, তবে কাজটা জাস্ট পেন্ডিং থাকুক
//         console.log('Next user not found, keeping task as is.')
//       }

//       alert(
//         "✅ You have skipped this task. It's now assigned to the next member."
//       )
//       await getUserInfo()
//     } catch (error: any) {
//       console.error('Error skipping task:', error.message)
//       alert('Failed to skip task: ' + error.message)
//     } finally {
//       setCompletingTaskId(null)
//     }
//   }
...
//       const { data: authData } = await supabase.auth.getUser()
//       const authUser = authData?.user
//       if (!authUser || !authUser.id) throw new Error('User ID is missing.')
...
//       const { data: authData } = await supabase.auth.getUser()
//       const authUser = authData?.user
//       if (!authUser || !authUser.id) throw new Error('Not logged in')

//       // ১. ইনভাইট কোড দিয়ে গ্রুপ খোঁজা
//       const { data: groupData, error: groupError } = await supabase
//         .from('groups')
...
//       // ২. নতুন মেম্বারের জন্য সিরিয়াল বানানো
//       const newSerial = groupData.total_members + 1

//       // ৩. ইউজারের প্রোফাইল আপডেট করে মেসে ঢুকিয়ে নেওয়া (role: member)
...
//       // ৪. গ্রুপের total_members ১ বাড়িয়ে দেওয়া
...
//       // ৫. সব ঠিক থাকলে ড্যাশবোর্ড রিলোড করা
//       await getUserInfo()
//     } catch (error) {
//       console.error('Failed to join group:', error)
//       alert('Failed to join the mess. Please try again.')
//     } finally {
//       setIsJoining(false)
//     }
//   }
...
//   // 🌟 যদি ইউজারের কোনো গ্রুপ না থাকে
...
//             {/* 🌟 Toggle Views: যদি isJoinMode True হয় তবে জয়েন ফর্ম দেখাবে, নইলে Create Option দেখাবে */}
//             {!isJoinMode ? (
//               <>
//                 {/* Create Group UI */}
...
//                       className="rounded-lg border border-gray-200 bg-white p-3 transition-all outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900"
//                       required
//                     />
//                     <Button
//                       type="submit"
//                       disabled={isCreating}
//                       className="text-md w-full bg-blue-600 py-6 font-bold shadow-md hover:bg-blue-700"
//                     >
//                       {isCreating ? (
//                         <>
...
//                   <span className="mx-4 flex-shrink-0 text-sm font-medium text-gray-400">
//                     OR
//                   </span>
//                   <div className="flex-grow border-t border-gray-200 dark:border-gray-700"></div>
//                 </div>
...
//                     Ask your mess admin for the 6-digit Join Code.
//                   </p>
//                   <Button
//                     variant="outline"
//                     onClick={() => setIsJoinMode(true)} // এখানে ক্লিক করলে ফর্ম পাল্টে যাবে
//                     className="w-full border-emerald-200 bg-white py-6 text-emerald-700 hover:bg-emerald-50 hover:text-emerald-800 dark:bg-gray-900"
//                   >
//                     Go to Join Page
//                   </Button>
//                 </div>
//               </>
//             ) : (
//               // Join Group Form UI (when isJoinMode is true)
//               <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
//                 <Button
//                   variant="ghost"
//                   onClick={() => setIsJoinMode(false)} // পেছনে যাওয়ার বাটন
...
//                   <h3 className="mb-3 flex items-center gap-2 font-bold text-gray-800 dark:text-gray-200">
//                     <Users className="h-5 w-5 text-emerald-600" /> Enter Join
//                     Code
//                   </h3>
//                   <form
...
//                       className="rounded-lg border border-emerald-200 bg-white p-4 text-center font-mono text-2xl font-bold tracking-widest uppercase transition-all outline-none placeholder:font-sans placeholder:text-base placeholder:font-normal placeholder:tracking-normal placeholder:text-gray-300 focus:ring-2 focus:ring-emerald-500 dark:border-gray-600 dark:bg-gray-900"
//                       maxLength={6}
//                       required
//                     />
//                     <Button
...
//                       className="text-md w-full bg-emerald-600 py-6 font-bold shadow-md hover:bg-emerald-700"
//                     >
//                       {isJoining ? (
//                         <>
//                           <Loader2 className="mr-2 h-4 w-4 animate-spin" />{' '}
...
//   // 🌟 যদি ইউজারের গ্রুপ থাকে (Dashboard)
//   return (
...
//               <Badge
//                 variant="default"
//                 className="bg-blue-600 px-3 py-1 shadow-sm hover:bg-blue-700"
//               >
//                 Admin
//               </Badge>
//             ) : (
//               <Badge
//                 variant="secondary"
//                 className="bg-emerald-100 px-3 py-1 text-emerald-800 shadow-sm hover:bg-emerald-200"
//               >
//                 Member
//               </Badge>
//             )}
...
//                   <div
//                     key={task.id}
//                     className="flex items-center justify-between rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800/50"
//                   >
...
//                             >
//                               <CheckCircle2 className="mr-1 h-3 w-3" /> Proxy
//                               Complete
//                             </Badge>
...
//                             size="sm"
//                             onClick={() =>
//                               handleCompleteTask(task.id, task.assigned_to)
//                             }
...
//                               variant="outline"
//                               onClick={() =>
//                                 handleSkipTask(task.id, user?.group_id || '')
//                               }
//                               disabled={
//                                 completingTaskId === task.id ||
//                                 (user?.skip_credit || 0) <= 0
//                               }
//                               className="h-8 border-amber-500 text-amber-600 hover:bg-amber-50"
//                             >
//                               <Gift className="mr-1 h-3 w-3" /> Skip with 1
//                               Credit
//                             </Button>
//                           ) : (
//                             ''
